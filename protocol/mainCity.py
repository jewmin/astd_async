...
async def mainCity(account: 'Account', result: 'ServerResult'):
    if result.success:
        account.user.UpdateCityInfo(result.result)


@ProtocolMgr.Protocol("升级建筑", ("player_BuildingId",))
async def upgradeLevel(account: 'Account', result: 'ServerResult', player_BuildingId, main_city_dto: 'MainCityDto'):  # noqa: F405
    if result.success:
...
async def getPerDayReward(account: 'Account', result: 'ServerResult'):
    if result.success:
...
async def getLoginRewardInfo(account: 'Account', result: 'ServerResult'):
    if result.success:
...
async def getReward(account: 'Account', result: 'ServerResult', cId, opt):
    if result.success:
        account.logger.info("领取登录送礼, 获得%s", RewardInfo(result.result["rewardinfo"]))  # noqa: F405


@ProtocolMgr.Protocol("争霸风云榜")
async def getChampionInfo(account: 'Account', result: 'ServerResult'):
    if result.success:
...
async def visitChampion(account: 'Account', result: 'ServerResult'):
    if result.success:
        account.logger.info("恭贺, 获得点券+%s", Format.GetShortReadable(result.result["tickets"]))


@ProtocolMgr.Protocol("将军塔")
async def getGeneralTowerInfo(account: 'Account', result: 'ServerResult'):
    if result.success:
...
async def useBuildingStone(account: 'Account', result: 'ServerResult', buildMode):
    if result.success:
        account.user.generaltower.HandleXml(result.result["generaltower"])
...
async def rightArmy(account: 'Account', result: 'ServerResult'):
    if result.success:
...
async def draught(account: 'Account', result: 'ServerResult', forceNum):
    if result.success:
        account.logger.info("征兵, 兵力+%s", Format.GetShortReadable(forceNum))


@ProtocolMgr.Protocol("征收")
async def perImpose(account: 'Account', result: 'ServerResult'):
...
async def selectLE(account: 'Account', result: 'ServerResult', opt, desc):
    if result.success:
        le = LeDto()  # noqa: F405
        le.HandleXml(result.result["ledto"])
        account.logger.info("%s, 选择答案[%d], 获得%s", desc, opt, le)
...
async def impose(account: 'Account', result: 'ServerResult'):
    if result.success:
...
async def forceImpose(account: 'Account', result: 'ServerResult'):
    if result.success:
...
async def resetSoldier(account: 'Account', result: 'ServerResult', generalId, generalName, forceNum):
    if result.success:
        account.logger.info("把武将[%s]的兵力设置为%d", generalName, forceNum)
