    ...
    async def _Exec(self):
        ...
        if info["翻牌次数"] > 0:
            ...
            if info["卡牌"][0]["gemlevel"] >= self.GetConfig("superlv"):
                buy_all = True
            if buy_all and info["卡牌全开花费金币"] <= self.GetConfig("buyall") and self.IsAvailableAndSubGold(info["卡牌全开花费金币"]):
                ...
                return self.immediate
        elif info["购买次数花费金币"] <= self.GetConfig("buyone") and self.IsAvailableAndSubGold(info["购买次数花费金币"]):
